...
def _admin_say_tell(adminid, message, teamonly):
    ...
    if userid: 
        ...
        if not teamonly: 
            ...
            xaextendedsay.logging.log("Admin %s has said '%s' to player %s" % (tokens['adminname'], message, tokens['username']) ) 
        else: 
...
def _admin_say_center(userid, message, teamonly): 
    ...
    if not teamonly: 
        ...
        xaextendedsay.logging.log("Admin %s has said '%s' in center chat" % (tokens['username'], message ) ) 
    else: 
        ...
        xaextendedsay.logging.log("Admin %s has said '%s' in admin only center chat" % (tokens['username'], message ) ) 
    return (0,'',0)
